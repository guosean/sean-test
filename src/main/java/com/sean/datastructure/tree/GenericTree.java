...
public final class GenericTree<T> {
     ...
     * @return The new tree.
     */
    public static <S> GenericTree<S> empty() {
        return of((S) null);
    }
     ...
     * @return The new tree.
     */
    public static <S> GenericTree<S> of(S value) {
     ...
     * @return The new tree.
     */
    public static <S> GenericTree<S> of(Node<S> node) {
        return node.asTree();
    }
     ...
     */
    public static <S> List<GenericTree<S>> of(Collection<S> collection, BiPredicate<Node<S>, Node<S>> parentChildMatcher) {
     ...
     * @return All of the parentless nodes after a changeless pass of the predicate turned into trees.
     */
    public static <S> GenericTree<S> of(Node<S> root, Collection<S> tail, BiPredicate<Node<S>, Node<S>> parentChildMatcher) {
    ...
    private GenericTree(Node<T> root, boolean cloneStructure) {
     ...
     */
    public final GenericTree<T> shallowClone() {
     ...
     * the original data references.
     */
    public final GenericTree<T> deepClone(Function<T, T> dataCloningFunction) {
        return new GenericTree<>(asNode().map(dataCloningFunction), false);
    }
     ...
     */
    public final GenericTree<T> sort(Comparator<T> sort) {
     ...
     * @return The same tree, but now with sorted levels.
     */
    public final GenericTree<T> sortAsNodes(Comparator<Node<T>> sort) {
     ...
     */
    public final <S> GenericTree<S> map(Function<T, S> func) {
        return new GenericTree<>(root.map(func), false);
    }
     ...
     */
    public final <S> GenericTree<S> flatMap(Function<T, Node<S>> func) {
        return new GenericTree<>(root.flatMap(func), false);
    }
     ...
     * @return The tree with new nodes.
     */
    public final <S> GenericTree<S> mapAsNodes(Function<Node<T>, S> func) {
        return new GenericTree<>(root.mapAsNode(func), false);
    }
     ...
     * @return The tree of new nodes.
     */
    public final <S> GenericTree<S> flatMapAsNodes(Function<Node<T>, Node<S>> func) {
        return new GenericTree<>(root.flatMapAsNode(func), false);
    }
     ...
     */
    public final GenericTree<T> filter(Predicate<T> predicate) {
     ...
     * @param predicate The predicate to identify nodes that should be removed.
     */
    public final GenericTree<T> filterAsNodes(Predicate<Node<T>> predicate) {
     ...
     * @param predicate The predicate to identify nodes that should be removed.
     */
    public final GenericTree<T> prune(Predicate<T> predicate) {
     ...
     * @param predicate The predicate to identify nodes that should be removed.
     */
    public final GenericTree<T> pruneAsNodes(Predicate<Node<T>> predicate) {
        depthFirstStreamNodes()
                .filter(node -> node != root && predicate.test(node))
    ...
    /**
     * 根据深度优先遍历，并返回包含目标元素的搜索路径列表
     *
     * @param value
     * @return
     */
    public final List<T> depthFirstSearchOfList(T value) {
        AtomicBoolean find = new AtomicBoolean(false);
        Predicate predicate = Predicate.isEqual(value);
        Optional<Node<T>> optionalTNode = depthFirstSearchNodes(s -> Objects.equals(value, s.getData()));
        Stack<T> stack = new Stack();

        while (optionalTNode.isPresent()) {
            stack.add(optionalTNode.get().getData());
            optionalTNode = optionalTNode.get().getParent();
        }
    ...
    public final boolean equals(Object o) {
        ...
        if (!(o instanceof GenericTree)) {
            return false;
        }

        GenericTree<T> tree = (GenericTree<T>) o;
    ...
    public static final class Node<T> {
         ...
         * @return
         */
        public final Node<T> addChildWithNode(T data) {
            Node<T> child = new Node<>(data);
            children.add(child);
            child.parent = this;
            return child;
         ...
         * the tree.
         */
        public final GenericTree<T> asTree() {
            return new GenericTree<>(this, true);
        }
        ...
        public final int getGlobalOrder() {
            return indexOfByRef(new GenericTree<>(getRootOfTree(), false)
    ...
    private static <S> int recursiveTreeNodeHashCode(Node<S> node) {
        return Arrays.hashCode(IntStream.concat(IntStream.of(Objects.hash(node)),
                node.children.stream().mapToInt(GenericTree::recursiveTreeNodeHashCode)).toArray());
    }
