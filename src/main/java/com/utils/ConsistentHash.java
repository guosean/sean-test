...
public class ConsistentHash<T> {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 10; i++) {
            tableList.add(String.valueOf(i));
        }
        
    	ConsistentHash<String> ch = new ConsistentHash<String>(500, tableList);
        ...
        for(int i =0; i<1000000; i++){
			...
			String hashCode = ch.get(sb.toString());
			if(m.get(hashCode) == null){
				m.put(hashCode, 1L);
			}else{
				Long value = m.get(hashCode) + 1;
				m.put(hashCode, value);
			}
        }
        System.out.println("耗时：" + (System.currentTimeMillis() - start));
        for(Map.Entry<String, Long> entry : m.entrySet()){
        	System.out.println(entry.getKey() + "=" + entry.getValue()+"===================="+Math.abs(entry.getValue()-100000));
        }
	}
}